                                             ...
                                             uint8_t qos )
{
    ...
    LogDebug( ( "Attempting to publish MQTT status message: "
                "message=%s",
                pMsg ) );
ESP_LOGI("publishStatusMessage", "pTopicBuffer:\n%s", pTopicBuffer);    
ESP_LOGI("publishStatusMessage", "pMsg:\n%s", pMsg);    
                                             ...
                                             const OtaFileContext_t * pOTAFileCtx )
{
    ...
    if( ( received == 1U ) || ( ( received % ( uint32_t ) otaconfigOTA_UPDATE_STATUS_FREQUENCY ) == 0U ) )
    {
        ...
        msgSize = ( uint32_t ) stringBuilder(
            ...
            msgBufferSize,
            payloadStringParts );
ESP_LOGI("buildStatusMessageReceiving", "pMsgBuffer:\n%s", pMsgBuffer);
        /* The buffer is static and the size is calculated to fit. */
...
LogInfo( ( "initFileTransfer_Mqtt pRxStreamTopic: %s", pRxStreamTopic ) );
...
ESP_LOGI("requestFileBlock_Mqtt 2", "pMsg = %s", pMsg);
// ESP_LOG_BUFFER_HEX("requestFileBlock_Mqtt 2", pMsg, sizeof( pMsg ));
// print_hex_spaced(pMsg,msgSizeFromStream);
// print_hex_spaced(pMsg,sizeof( pMsg ));
    if( cborEncodeRet == true )
    {
        ...
        topicLen = ( uint32_t ) stringBuilder(
            ...
            sizeof( pTopicBuffer ),
            pTopicParts );
ESP_LOGI("requestFileBlock_Mqtt", "pTopicBuffer = %s", pTopicBuffer);
        /* The buffer is static and the size is calculated to fit. */
        assert( ( topicLen > 0U ) && ( topicLen < sizeof( pTopicBuffer ) ) );
